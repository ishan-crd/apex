...
function SpinnerIcon({ color = C.accent }: { color?: string }) {
  return (
...
export default function AnalyzeScreen() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setPct(current);
      if (current % 25 === 0) {
        setCapIdx((i) => Math.min(i + 1, SCAN_CAPTIONS.length - 1));
      }
      if (current >= 100) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {/* Fig placeholder */}
      <View style={styles.figPlaceholder}>
      ...
      <View style={styles.body}>
        ...
        <Animated.Text style={[styles.caption, s(capAnim)]} key={capIdx}>
          {SCAN_CAPTIONS[capIdx]}
        </Animated.Text>
        ...
        <Animated.View style={[{ opacity: ctaAnim }, s(ctaAnim)]}>
          <View
            onTouchEnd={() => done && router.push('/onboarding-plan')}
            style={[styles.cta, !done && { opacity: 0, pointerEvents: 'none' } as any]}
          >
