...
function CheckIcon() {
  const C = useColors();
  return <Svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={C.accentInk} strokeWidth={2.4} strokeLinecap="round" strokeLinejoin="round"><Path d="M5 12.5 10 17l9-10" /></Svg>;
}
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      const step = () => {
        ...
        if (current >= target) { setVal(target); } else { setVal(current); requestAnimationFrame(step); }
      };
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.body}>
        ...
        <Animated.View style={s(d4)}>
          <TouchableOpacity activeOpacity={0.85} onPress={() => router.replace('/(tabs)/dashboard')} style={styles.cta}>
            <CheckIcon /><Text style={styles.ctaText}>Let's go</Text>
          </TouchableOpacity>
        </Animated.View>
      </View>
    </SafeAreaView>
